...
document.addEventListener("DOMContentLoaded", async () => {
    console.log("Session Storage loginID:", sessionStorage.getItem('loginID'));

    var forums = [];
    var userArray, forumArray, loginStatus;
    ...
    if (document.cookie) {
        ...
        userCookie.forEach(cookie => {
            ...
            if (key === 'userInfo') {
                userArray = value;
                console.log("Extracted userArray:", userArray);
            } else if (key === 'forumInfo') {
                forumArray = value;
                console.log("Extracted forumArray:", forumArray);
            } else if (key === 'loginStatus') {
                loginStatus = value;
                console.log("Extracted loginStatus:", loginStatus);
            }
        });
        ...
        if (forumArray) {
            ...
            console.log("Decoded forumArray:", split);
            console.log("bukaka Array:", bukaka);

            var bukakaMore;
            var i = 0;

            while (i < bukaka.length && bukaka[i] !== '') { 
                bukakaMore = bukaka[i].replaceAll('[', '').replaceAll('"', '');
                forums.push(bukakaMore.split(',').filter(Boolean));
                i++;
            }
        } else {
            console.warn("forumInfo cookie not found, attempting to fetch forums...");
            try {
                const forumRes = await fetch('/getForums');
                const forumsData = await forumRes.json();
                console.log("Fetched forums from server:", forumsData);
                forumArray = JSON.stringify(forumsData);
                forums = forumsData;
            } catch (err) {
        ...
        console.warn("No cookies found, fetching user and forum data...");
        try {
    ...
    if (loginStatus === 'true' && sessionStorage.getItem('loginID') === userArray?.split(',').filter(Boolean)[1]) {
        ...
        const userObject = userArray.split(',').filter(Boolean)[0];
        console.log("User Object:", userObject);

        console.log("User Array:", userArray.split(',').filter(Boolean));
        console.log("User ID:", userArray.split(',').filter(Boolean)[0]);
        ...
        console.warn("Invalid login status, refetching data...");
        try {
            const userRes = await fetch('/getUsers');
            const forumRes = await fetch('/getForums');
            const loginRes = await fetch('/getLoggedIn');
